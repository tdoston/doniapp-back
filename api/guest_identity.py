...
def ensure_guest_schema(cursor) -> None:
    ...
    if not cursor.fetchone():
        ...
        cursor.execute("""
            ...
            WHERE table_schema = 'public' AND table_name = 'guests'
        """)
        gcols = {row[0] for row in cursor.fetchall()}
    ...
    with connection.cursor() as c:
        c.execute(
            ...
            WHERE h.name = %s AND b.status = 'active'
              ...
              AND b.check_in_date::date <= %s::date + ((%s - 1) || ' days')::interval
              AND %s::date <= b.check_in_date::date + ((b.nights - 1) || ' days')::interval
            LIMIT 1
            """,
            [
                hostel_name,
